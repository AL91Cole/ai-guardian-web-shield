...
(() => {
  ...
  const WEBMAIL_TITLE_HINTS = ["inbox", "mail", "webmail", "message", "messages", "mailbox", "outlook", "roundcube"];
  ...
  const PROVIDER_CONFIGS = {
    ...
    outlook: {
      ...
      detection: {
        ...
        uiSelectors: ["button[aria-label*='New mail' i]", "div[role='tree']", "[aria-label*='Outlook' i]"],
  ...
  function hasProviderUiCues(environment) {
    ...
    if (
      ...
      domUtils.countVisibleMatches(getProviderUiSelectors(environment), 8) > 0
    ) {
      return true;
    }

    return getProviderUiTextHints(environment).some((hint) => titleText.includes(hint) || bodySample.includes(hint));
  }
